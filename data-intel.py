...
def parse_mutilate_file(filepath):
    ...
    avg_lat = p90_lat = p99_lat = total_qps = None
...
def collect_logs_from_directory(run_dir):
    ...
    for root, _, files in os.walk(run_dir):
        mutilate_files = [f for f in files if "mutilate" in f and f.endswith(".log")]
        powerstat_files = [f for f in files if "powerstat" in f]
        mutilate_rates = []
        powerstat_rates = []
